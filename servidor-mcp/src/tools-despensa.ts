/**
 * As ferramentas da despensa do consumidor.
 *
 * Vivem no Worker de Compras (compras-mcp). O lado lojista esta em
 * tools-encartes.ts e sobe no Worker de Mercado, sobre o mesmo D1.
 */
...
export function registrarToolsDespensa(server: McpServer, db: D1Database, userId: string) {
	// ------------------------------------------------------------------ READ
	server.registerTool(
		"despensa_listar",
		{
