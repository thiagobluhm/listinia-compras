...
Delegue ao agente **`mercado-orquestrador`** (ferramenta `Agent`). Ele
encadeia `mercado-leitor-planilha` → prévia → `mercado-publicador`.

## A missão

- o pedido do lojista, nas palavras dele;
- **o caminho do arquivo anexado** — os subagentes não enxergam anexo, abrem
  pelo caminho;
- a vigência, se ele já disse.

## O que o orquestrador precisa garantir
...
`${CLAUDE_PLUGIN_ROOT}/references/contrato-encarte.md`.
